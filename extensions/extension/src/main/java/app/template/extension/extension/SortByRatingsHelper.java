...
public final class SortByRatingsHelper {
    ...
    private static final String SORT_RATINGS_JS =
        ...
        + "var cands=card.querySelectorAll('[data-csa-c-content-id=\"alf-customer-ratings-count-component\"] span,"
        + "a[href*=\"customerReviews\"] span,"
        + "a[href*=\"#customerReviews\"],span.a-size-base.s-underline-text,"
        + "span.a-size-small span,span.a-size-base,span.a-size-mini');"
        ...
        + "var cards=collect();"
        + "var scored=[];"
        + "for(var i=0;i<cards.length;i++){var el=cards[i];"
        + "var star=!!el.querySelector('[aria-label*=\"out of\"],.a-icon-star');"
        + "if(!star)continue;"
        + "scored.push({el:el,c:cardCount(el),i:i});}"
        + "if(scored.length<2)return 0;"
        + "scored.sort(function(a,b){if(b.c!==a.c)return b.c-a.c;return a.i-b.i;});"
        + "var parent=scored[0].el.parentNode;"
        + "var same=[];"
        + "for(var i=0;i<scored.length;i++){if(scored[i].el.parentNode===parent)same.push(scored[i]);}"
        + "if(same.length<2)return 0;"
        + "if(!parent.getAttribute('data-morphe-orig')){"
        + "parent.setAttribute('data-morphe-orig',parent.style.display||'');"
        + "parent.style.display='flex';parent.style.flexDirection='column';}"
        + "for(var k=0;k<same.length;k++){var m=same[k];"
        + "if(!m.el.hasAttribute('data-morphe-ord'))"
        + "m.el.setAttribute('data-morphe-ord',m.el.style.order||'');"
        + "m.el.style.order=k+1;}"
        + "return same.length;}"
        + "function restore(){"
        + "var cards=collect();"
        + "for(var i=0;i<cards.length;i++){var el=cards[i];"
        + "if(el.hasAttribute('data-morphe-ord')){"
        + "el.style.order=el.getAttribute('data-morphe-ord');"
        + "el.removeAttribute('data-morphe-ord');}}"
        + "var touched=document.querySelectorAll('[data-morphe-orig]');"
        + "for(var i=0;i<touched.length;i++){"
        + "touched[i].style.display=touched[i].getAttribute('data-morphe-orig');"
        + "touched[i].removeAttribute('data-morphe-orig');}}"
        + "function ensureBtn(){"
