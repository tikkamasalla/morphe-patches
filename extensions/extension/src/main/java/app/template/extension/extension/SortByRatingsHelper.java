...
public final class SortByRatingsHelper {
    ...
    private static final String SORT_RATINGS_JS =
        ...
        + "return p;}"
        + "function groupParent(cards){"
        + "var groups=[];"
        + "for(var i=0;i<cards.length;i++){var p=cards[i].parentNode;var g=null;"
        + "for(var j=0;j<groups.length;j++){if(groups[j].p===p){g=groups[j];break;}}"
        + "if(!g){g={p:p,els:[]};groups.push(g);}"
        + "g.els.push({el:cards[i],i:i});}"
        + "var best=null;"
        + "for(var j=0;j<groups.length;j++){"
        + "if(!best||groups[j].els.length>best.els.length)best=groups[j];}"
        + "return best;}"
        + "function dbgDump(){"
        + "var all=collect();var html='CARDS:'+all.length+'<br>';"
        + "for(var i=0;i<Math.min(all.length,3);i++){var el=all[i];"
        + "html+='---card'+i+' star:'+!!el.querySelector('[aria-label*=\"out of\"],.a-icon-star')+'<br>';"
        + "var ss=el.querySelectorAll('span');"
        + "for(var j=0;j<Math.min(ss.length,25);j++){"
        + "var t=(ss[j].textContent||'').trim().split(/\\s+/).join(' ').slice(0,60);"
        + "if(t)html+=(j+':['+String(ss[j].className||'').slice(0,40)+'] '+t+'<br>');}}"
        + "var d=document.getElementById('morphe-dbg');"
        + "if(!d){d=document.createElement('div');d.id='morphe-dbg';"
        + "d.style.cssText='position:fixed;left:8px;top:60px;max-width:94%;max-height:70%;"
        + "overflow:auto;z-index:2147483647;background:#000;color:#0f0;font-size:10px;"
        + "padding:8px;white-space:pre-wrap;';document.documentElement.appendChild(d);}"
        + "d.innerHTML=html;}"
        + "function applySort(){"
        + "dbgDump();var cards=collect();"
        + "if(cards.length<2)return 0;"
        + "var grp=groupParent(cards);"
        + "if(!grp||grp.els.length<2)return 0;"
        + "var parent=grp.p;"
        ...
        + "var mapped=grp.els.map(function(o){return{el:o.el,c:cardCount(o.el),i:o.i};});"
